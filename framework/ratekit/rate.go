...
const RateChanSize 					= 10000             //chan长度
const DefaultTaskNumEverySecond 	= 100 				//默认每秒跑多少个任务
const MaxTaskNumEverySecond 		= 10000   			//最大速度
...
//limitcount 每秒钟运行次数上限制， WorkerMostNum工作go程数量， issync是否是同步模式
func NewRateKit(limitCount uint32) *RateKit {
	...
	//启动异步任务go程
	rk.factory = NewFactory(rk.asyncTask, rk.wait)
	rk.factory.adjust(10)
...
func (rk *RateKit) flush() {
	...
	for {
		...
		if currentCount < limitCountPerSecond {
			//加更多的worker
			lenChan := len(rk.asyncChan)
			rk.rs.Put(lenChan)

			if lenChan > 10 {
				runningNum := atomic.LoadUint32(&rk.factory.runningNum)
				add := (WorkerMostNum-runningNum)/20 + 5
				rk.factory.adjust(runningNum + add)
			} else {
				//最近几次的sum都是0
				if rk.rs.Sum() == 0 {
					...
					runningNum := atomic.LoadUint32(&rk.factory.runningNum)
					if runningNum > 3 {
						rk.factory.adjust(runningNum - 2)
					}
				}
			}
			...
			//-worker的数量
			runningNum := atomic.LoadUint32(&rk.factory.runningNum)
			rk.factory.adjust(runningNum - reduce)
		} else {
			//不变
		}
...
func (rk *RateKit) Go(f func() bool, leftTryCount uint32) {
	...
	if len(rk.asyncChan) > RateChanSize/2 {
		limitCountPerSecond := atomic.LoadUint32(&rk.limitCountPerSecond)
		duration = (uint32(1000*1000) / limitCountPerSecond) * 2
	} else if len(rk.asyncChan) > (RateChanSize/2 + RateChanSize/4) {
		limitCountPerSecond := atomic.LoadUint32(&rk.limitCountPerSecond)
...
func (rk *RateKit) asyncTask() bool {
	rf := <-rk.asyncChan
	f := rf.f
...
func (rk *RateKit) CollectInfo() {
	...
	str += "队列长度:" + strconv.Itoa(len(rk.asyncChan)) + "\n"
...
/***************************************worker*******************************/
const StatusWorkerRunning 		= 1 			//正在运行
const StatusWorkerNotRunning 	= 0
const WorkerMostNum 			= 1000
...
func NewFactory(f func() bool, wait *sync.WaitGroup) *Factory {
	...
	if WorkerMostNum <= 0 {
		fmt.Println("不能设置WorkerMostNum小于等于零")
		os.Exit(1)
	}
	
	fac.workerList = make([]*Worker, WorkerMostNum, WorkerMostNum)
	
	for i := 0; i < WorkerMostNum; i++ {
		worker := &Worker{}
		worker.task = f
		worker.running = StatusWorkerNotRunning
		worker.factory = fac
	...
	fac.bb.InitLogKit("worker_panic", "worker_start", "worker_end", "adjust_num")
	...
	fac.bb.SetLogReadme("adjust_num", "调整工作goroutine数量")
...
//调整worker启动数量为num
func (fac *Factory) adjust(num uint32) {
	if num <= 0 {
		num = 1
	}

	if num >= WorkerMostNum {
		num = WorkerMostNum
	}
...
func (fac *Factory) addOneRunning() {
	if fac.runningNum == WorkerMostNum {
		return
	}
	...
	if r > 5 {
		//找到第一个没有运行的，并启动
		for i := 0; i < WorkerMostNum; i++ {
			if atomic.CompareAndSwapInt32(&(fac.workerList[i].running), StatusWorkerNotRunning, StatusWorkerRunning) {
				...
				fac.bb.Log("adjust_num", "调整worker数量,增加一个worker,正序")
				break
			}
		}
	} else {
		for i := WorkerMostNum - 1; i >= 0; i-- {
			if atomic.CompareAndSwapInt32(&(fac.workerList[i].running), StatusWorkerNotRunning, StatusWorkerRunning) {
				fac.bb.Log("worker_start", "启动 worker")
				go fac.workerList[i].run() //启动
				atomic.AddUint32(&fac.runningNum, 1)
				fac.bb.Log("adjust_num", "调整worker数量,增加一个worker,反序")
				break
			}
		}
	}
...
func (fac *Factory) reduceOneRunning() {
	...
	//找到第一个运行的，并关闭
	for i := 0; i < WorkerMostNum; i++ {
		if atomic.CompareAndSwapInt32(&fac.workerList[i].running, StatusWorkerRunning, StatusWorkerNotRunning) {
			atomic.AddUint32(&fac.runningNum, ^uint32(0))
			fac.bb.Log("adjust_num", "调整worker数量,关闭一个worker")
			break
		}
	}
}
...
type Worker struct {
	...
	task    func() bool 	// 这里面不必要再加个无限循环，仅仅写业务逻辑就可以了,注意这是一个可以重复多次执行的函数，里面类似从某个chan拿出数据并且处理
...
func (w *Worker) run() {
	...
	for {
		...
		if atomic.CompareAndSwapInt32(&w.running, StatusWorkerNotRunning, StatusWorkerNotRunning) {
			//状态为停止，关闭worker
